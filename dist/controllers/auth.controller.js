...
import { generateAccessToken, geneateRefreshToken, verifyRefreshToken, verifyAccessToken, } from "../utils/jwt.js";
...
export const register = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        const result = await client.query(`INSERT INTO users (name, email, password) VALUES ($1, $2, $3) RETURNING id, name, email`, [name, email, hashedPassword]);
        const user = result.rows[0];
        const rawToken = crypto.randomBytes(32).toString("hex");
        const hashedToken = crypto.createHash("sha256").update(rawToken).digest("hex");
        await client.query(`INSERT INTO email_verification_token (user_id, token, expires_at) VALUES ($1, $2, NOW() + INTERVAL '30 minutes')`, [user.id, hashedToken]);
...
export const refresh = asyncHandler(async (req, res) => {
    ...
    const sessionResult = await pool.query(`SELECT * FROM sessions WHERE id = $1 AND refresh_token_hash = $2 AND revoked = false`, [decoded.sessionId, hashedToken]);
...
export const enableMFA = asyncHandler(async (req, res) => {
    ...
    if (!user || !user.mfa_secret)
        return res.status(400).json({ message: "MFA has not been setup." });
    const verified = speakeasy.totp.verify({ secret: user.mfa_secret, encoding: "base32", token, window: 1 });
    if (!verified)
...
export const forgotPassword = asyncHandler(async (req, res) => {
    ...
    const rawToken = crypto.randomBytes(32).toString("hex");
    const hashedToken = crypto.createHash("sha256").update(rawToken).digest("hex");
    await pool.query(`INSERT INTO password_reset_token (user_id, token, expires_at) VALUES ($1, $2, NOW() + INTERVAL '1 hour')`, [user.id, hashedToken]);
...
export const resetPassword = asyncHandler(async (req, res) => {
    ...
    const hashedToken = crypto.createHash("sha256").update(token).digest("hex");
    const tokenResult = await pool.query("SELECT * FROM password_reset_token WHERE token = $1 AND expires_at > NOW()", [hashedToken]);
